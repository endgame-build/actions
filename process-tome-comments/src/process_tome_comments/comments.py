...
class Comment:
    ...
    def resolved(self, *, by: str, at: str) -> Comment:
        ...
        return replace(
            self, is_resolved=True, resolved_by=by, resolved_at=at, updated_at=at
        )
...
class Cluster:
    ...
    def with_block_location(self, source: str) -> Cluster:
        ...
        return replace(
            self, block_snippet=snippet, line_start=line_start, line_end=line_end
        )
    ...
    def from_dict(cls, d: dict[str, Any]) -> Cluster:
        comments = sorted(
            (Comment.from_dict(c) for c in d["comments"]),
