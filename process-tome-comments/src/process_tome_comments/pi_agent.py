...
class PiAgent:
    ...
    def _build_prompt(self, cluster: Cluster) -> str:
        if cluster.line_start is not None and cluster.line_end is not None:
            location = f"lines {cluster.line_start}–{cluster.line_end}, block index {cluster.block_index}"
        else:
        ...
        comments_md = "".join(
            f"- `{c.id}` — @{c.author_login} ({c.created_at}): {c.body}\n"
            for c in cluster.comments
        )

        return f"""{self.prelude_text}
---

**Source:** `{cluster.file_path}` ({location})
{snippet_block}
**Comments to address (in arrival order):**
