...
**Idempotency filter** — the pre-cluster check that drops any comment whose `id` already has an **open** bot PR. Prevents opening a duplicate while one is in flight. Implemented via `TomeBacklog`: a single `gh pr list --state open` call returns the labels of every open tome-PR; comment ids are extracted from `tome-cid:<uuid>` labels and the filter is an in-memory set lookup.

The canonical "discard this comment" signal is resolving it in Tome (`isResolved: true` in `.tome/comments.jsonl`) — the unresolved-filter drops it before clustering. Closing a bot PR without merging is the "this attempt wasn't good enough, try again on next dispatch" signal and does *not* mark the comment as addressed.

**Tome backlog** — snapshot of the repo's currently-open tome-PRs, built once per `prepare` invocation. Defined in `backlog.py:TomeBacklog`. Carries the set of comment ids addressed by an open PR and the count of currently-open tome-PRs. Answers both questions `prepare` asks the GitHub API (idempotency + slot budget) from one fetch.

**`auto:tome-comment-pr`** — the common label applied to every tome-PR at open time. Enables the single `label:"auto:tome-comment-pr"` search that powers `TomeBacklog`. The per-id `tome-cid:*` labels stay forever — `consolidate` reads them on merge to decode which comment ids to resolve in `.tome/comments.jsonl`.

**Disallowed paths** — paths the agent must not modify: `.github/` (would break the workflow's own next run) and `.tome/comments.jsonl` (owned by `consolidate`'s post-merge resolution writes). Enforced post-edit by `policy.py:policy_violations`, which runs `git diff --cached --name-only` and returns any matching paths. If non-empty, the cluster is aborted and the working tree restored.
