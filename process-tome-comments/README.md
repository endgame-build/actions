...
├── scripts/                        # bash; requires jq + gh on the runner (Ubuntu defaults provide both)
│   ├── prepare-clusters.sh         # filter + cluster + slot budget + emit matrix
│   ├── build-cluster-prompt.sh     # concatenate prelude + cluster context
│   ├── snapshot-and-pr.sh          # validate, branch/commit/push, open PR
│   └── consolidate.sh              # post-merge JSONL update
